...
#define FORTEAMS(T) for(int T = TEAM_RED; T != -1; T = (T==TEAM_RED?TEAM_BLUE:-1))
 ...
 #define D(F, ...) dbg_msg("BoloFNG", "%s:%i:%s(): " F, __FILE__, __LINE__, \
                                                            __FUNCTION__, __VA_ARGS__)
#elif defined(CONF_FAMILY_UNIX)
 #define D(F, ARGS...) dbg_msg("BoloFNG", "%s:%i:%s(): " F, __FILE__, __LINE__, \
                                                            __func__,##ARGS)
#endif
...
CGameControllerBoloFNG::~CGameControllerBoloFNG()
{
	Reset(true);
}

void CGameControllerBoloFNG::Reset(bool Destruct)
{
...
bool CGameControllerBoloFNG::DoEmpty()
{
	bool Empty = true;
...
void CGameControllerBoloFNG::DoHookers()
{
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
...
void CGameControllerBoloFNG::DoInteractions()
{
	DoHookers();
...
void CGameControllerBoloFNG::DoScoreDisplays()
{
	/*FORTEAMS(i)
...
void CGameControllerBoloFNG::DoBroadcasts(bool ForceSend)
{
	if (m_GameOverTick != -1)
		return;
...
void CGameControllerBoloFNG::DoRagequit()
{
	if (*m_aRagequitAddr)
	{
		char aBuf[128];
...
void CGameControllerBoloFNG::HandleFreeze(int Killer, int Victim)
{
...
void CGameControllerBoloFNG::HandleMelt(int Melter, int Meltee)
{
...
void CGameControllerBoloFNG::HandleSacr(int Killer, int Victim, int ShrineTeam)
...
void CGameControllerBoloFNG::SendFreezeKill(int Killer, int Victim, int Weapon)
{
	char aBuf[256];
...
/*
bool CGameControllerBoloFNG::CanBeMovedOnBalance(int ClientID)
{
...
*/
int CGameControllerBoloFNG::OnCharacterDeath(class CCharacter *pVictim, class CPlayer *pUnusedKiller, int Weapon)
{
...
void CGameControllerBoloFNG::PostReset()
{
...
bool CGameControllerBoloFNG::OnEntity(int Index, vec2 Pos)
{
	switch(Index)
	{
	case ENTITY_SPAWN:
...
bool CGameControllerBoloFNG::CanJoinTeam(int Team, int NotThisID)
{
...
CBroadcasterBolo::CBroadcasterBolo(class CGameContext *pGameServer)
: m_pGS(pGameServer)
{
	Reset();
}

CBroadcasterBolo::~CBroadcasterBolo()
{
	Reset();
}

void CBroadcasterBolo::SetDef(const char *pText)
{
...
void CBroadcasterBolo::Update(int Cid, const char *pText, int Lifespan)
{
	if (Cid < 0) // all
	{
		for(int i = 0; i < MAX_CLIENTS; ++i)
